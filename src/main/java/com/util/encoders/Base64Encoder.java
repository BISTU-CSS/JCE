...
public class Base64Encoder implements Encoder {
   ...
   public boolean isBase64(byte[] data, int off, int length) {
      for(int index = off; index < length; ++index) {
         if (data[index] != this.padding) {
            boolean found = false;

            for(int bin = 0; bin < this.encodingTable.length; ++bin) {
               if (data[index] == this.encodingTable[bin]) {
                  found = true;
                  break;
               }
            }
