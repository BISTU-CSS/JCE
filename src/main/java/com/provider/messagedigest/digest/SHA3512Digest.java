...
public class SHA3512Digest {
    ...
    public SHA3512Digest() {
        this.V = (byte[])((byte[])SHA3512DigestUtil.iv.clone());
        this.cntBlock = 0;
    }

    public SHA3512Digest(SHA3512Digest t) {
        this.V = (byte[])((byte[])SHA3512DigestUtil.iv.clone());
        this.cntBlock = 0;
    ...
    private void compute(byte[] block) {
        byte[] V1 = SHA3512DigestUtil.CF(this.V, block);
    ...
    private byte[] doFinal() {
        ...
        byte[] tmp = SHA3512DigestUtil.padding(buffer, this.cntBlock);
    ...
    private byte[] getSM2Za(byte[] x, byte[] y, byte[] id) {
        ...
        dPos += 32;
        SHA3512Digest digest = new SHA3512Digest();
